...
const grid = {
	...
	fields: [
		...
		{ fieldName: '시료확인', dataType: ValueType.TEXT },
	...
	columns: [
		{ name: "fCstno", fieldName: "fCstno", header: { text: "투입카세트" }, renderer: { showTooltip: true }, width: 80, styleName: "tl" },
		...
		{ name: "tCstno", fieldName: "tCstno", header: { text: "출고카세트" }, renderer: { showTooltip: true }, width: 80, styleName: "tl" },
		{ name: "t수량", fieldName: "t수량", header: { text: "양품수량" }, numberFormat: "#,##0", renderer: { showTooltip: true }, footer: {
		...
		{ name: "t불량수량", fieldName: "t불량수량", header: { text: "불량수량" }, numberFormat: "#,##0", renderer: { showTooltip: true }, footer: {expression: "sum"}, width: 50, styleName: "tr" },
		{ name: "qty", fieldName: "qty", header: { text: "불량/시료수량" }, numberFormat: "#,##0", footer: {expression: "sum"}, width: 50, styleName: "tr" },
		{ name: "불량입력", fieldName: "불량입력", header: { text: "불량" },
			renderer:{
				type:"html",
		...
		{ name: "시료확인", fieldName: "시료확인", header: { text: "QR Cell" },
			renderer:{
				type:"html",
				callback: function(grid, cell, w, h) {
					const ds = grid.getDataSource();
					const jsonData = ds.getJsonRow(cell.index.itemIndex);
					const str = '<button type="button" class="'+(jsonData['qty'] > 0 ? 'btn_sucess' : 'btn_error')+'">불량/시료</button>';
					return str;
				}
			},
		},
		{ name: "특이사항", fieldName: "특이사항", header: { text: "비고" }, renderer: { showTooltip: true }, width: 250, styleName: "tl" },
	],
