  ...
  <div>
    ...
    <div class="grid_box search_onerow">
      ...
      <div class="grid-border-none">
        <RealGrid ref="facGrid" :uid="'facGrid'" :rows="facRows" style="height: 100%" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    facGridView() {
      return this.$refs.facGrid.getGridView();
    },
  },
  created() {
    this.initializeGrid();
  ...
  methods: {
    initializeGrid() {
      this.facGrid = _.cloneDeep(require(`@web/m0003000/js/TAB044003.js`));
    },
    ...
    clearClick() {
      this.facGridView.resetFilters();
      this.facGridView.clearCurrent();
    ...
    searchClick() {
      ...
      this.$nextTick(async () => {
        ...
        let searchParam = [
          {
            menuId: 'M0003009',
            queryId: this.params.gubun === '미생성' ? 'M0003009_Sch3_1' : 'M0003009_Sch3',
            queryParams: params,
            target: this.facRows,
          },
        ];
    ...
    async excelClick() {
      ...
      const fileName = `포장/출하_내포장실적_실적DATA관리_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
    ...
    onDataLoadComplatedFacGrid(grid) {
