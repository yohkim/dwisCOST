    ...
    <div>
            ...
            <b-row class="search_area">
                ...
                <b-col cols="2">
                    <div class="form-floating">
                        ...
                        <label for="floatingSelect" class="select">자재종류</label>
                    </div>
                </b-col>
                <b-col cols="3">
                    <div class="form-floating">
        ...
        <div class="worker_wrap">
            <b-row class="worker_info">
                ...
                <b-col cols="4">
                    <div class="form-floating">
                        <input type="text" ref="materialId" class="form-control label-60" id="floating" placeholder="바코드" autocomplete="off" v-model="materialId" :disabled="this.searchSubMatType==='' || this.workerName===''" 
                ...
                <b-col cols="2">
                    <div style="width:300px"><span style="font-size: 11px; color: blue;">* 입고작업전 자재종류를 선택, 담당자를 입력하십시요. </span></div>
                </b-col>                
            </b-row>
        </div>
...
import gridField from '@web/m0001000/js/TAB001105.js';

const URI_PREFIX = '/api/m0001000/m0001005/TAB001105';
...
export default {
	...
	mounted() {
        ...
        this.$eventBus.on("TAB001105MenuMove", this.menuMove);
	},

	beforeUnmount() {
        this.$eventBus.off("TAB001105MenuMove");
    },
    beforeDestroy() {
    ...
    methods: {
        ...
        async getSelectOptions(){
			...
			let params = [ { majCode : "81"}, {matCategoryCode : MAT_CTG_CODE, matTypeCode : MAT_TYPE_CODE} ];
            let matInfo = [];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001005',queryId:'selectMatTrayIn', queryParams:params, target: this.gridRows};
        ...
        addRow(matInfo){
            ...
            this.matGridDataProvider.addRow({
                ...
                subMatTypeCode: this.searchSubMatType,
                model: matInfo.model,
                trayCode : matInfo.trayCode,
                unit : this.unit,
                inQty: matInfo.inQty,
                useQty: 0,
                remainQty: matInfo.inQty,
                useYn: 'N',
		...
		checkValidBarcodeData(checkData) {
            ...
            if(!_.isUndefined(dupRow)){
                this.$toast('info', "금일 이미 추가된 바코드입니다.");
                this.materialId = "";
                return false;
            }
        ...
        async processBarcode(){
            ...
            if ( this.checkValidBarcodeData() ) {
                let params = { materialId: this.materialId, trayCode: this.materialId.slice(0,10), inDate: this.$utils.getTodayDate("") };
                let trayInfo = [];

                let param = {menuId:'M0001005',queryId:'selectBarcodeModelInfo', queryParams:params, target:trayInfo};
                let resp = await this.$axios.api.search(param);

                if ( _.isEmpty(trayInfo[0].model)) {
                    this.$toast("info", "모델정보를 찾을 수 없습니다.\n[" + this.materialId + "] TrayCdoe별 모델정보를 등록하십시요!");
                    this.materialId = "";
                    return false;
                }
                else if ( trayInfo[0].dupYn === "Y" ) {
                    this.$toast("info", "금일 이미 입고 처리된 바코드입니다!");
                    this.materialId = "";
                    return false;
                }
                else {
                    this.addRow(trayInfo[0]);
                }                
        ...
        async excelBtnClick(){
			...
			const fileName = `Tray_공정전입고_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
