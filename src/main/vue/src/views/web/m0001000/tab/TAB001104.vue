...
import gridField from '@web/m0001000/js/TAB001104.js';

const URI_PREFIX = '/api/m0001000/m0001004/TAB001104';
...
export default {
	...
	mounted() {
        ...
        this.$eventBus.on("TAB001104MenuMove", this.menuMove);
	},

	beforeUnmount() {
        this.$eventBus.off("TAB001104MenuMove");
    },
    beforeDestroy() {
    ...
    methods: {
        ...
        async getSelectOptions(){
			...
			let params = [ { majCode : "82"}, { majCode : "74"}, {matCategoryCode : MAT_CTG_CODE, matTypeCode : MAT_TYPE_CODE} ];
            let matInfo = [];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001004',queryId:'selectMatFilmIn', queryParams:params, target: this.gridRows};
        ...
        async processBarcode(){
            ...
            if ( this.checkValidBarcodeData() ) {
                let params = { materialId: this.materialId, majCode:"82" };
                let filmInfo = [];

                let param = {menuId:'M0001004',queryId:'selectBarcodeModelInfo', queryParams:params, target:filmInfo};
                let resp = await this.$axios.api.search(param);

                if ( _.isEmpty(filmInfo[0].modelCode)) {
                    this.$toast("info", "모델정보를 찾을 수 없습니다.\n[" + this.materialId + "] 모델별 필름정보를 등록하십시요!");
                    this.materialId = "";
                    return false;
                }
                else if ( filmInfo[0].dupYn === "Y" ) {
                    this.$toast("info", "이미 입고 처리된 바코드입니다!");
                    this.materialId = "";
                    return false;
                }
                else {
                    this.addRow(filmInfo[0]);
                }                
        ...
        async excelBtnClick(){
			...
			const fileName = `필름_공정전입고_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
