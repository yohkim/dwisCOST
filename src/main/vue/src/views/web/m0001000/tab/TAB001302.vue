    ...
    <div>
        <div class="search_box" :class="{ disabled: isBarcodeMode }">
            <b-row class="search_area">
                ...
                <b-col cols="2">
                    <div class="form-floating">
                        <select class="form-select label-130 " id="floatingSelect" aria-label="Floating label select example" v-model="searchStoreValidPassYn" >
                            <option v-for="item in storeValidPassYnList" :value="item.itemValue" :key="item.itemText">{{item.itemText}}</option>
                        </select>
		...
		<div class="worker_wrap">
            <b-row class="worker_info">	
				<b-col cols="3" class="d-flex ms-3">
...
import gridField from '@web/m0001000/js/TAB001302.js';

const URI_PREFIX = '/api/m0001000/m0001002/TAB001302';
...
export default {
	...
	data () {
		return {
			...
			confirmYnList:[],
			diffYnList:[],
			storeValidPassYnList:[],
			curMode: 'R',
			searchStockDate:'',
			searchSubMatType:'',
			searchConfirmYn:'',
			searchDiffYn:'',
			searchStoreValidPassYn:'',
			stockStatus:'',
			stockStatusMsg:'',
	...
	mounted() {
		...
		this.$eventBus.on("TAB001302MenuMove", this.menuMove);		
	},
	beforeUnmount() {
		this.$eventBus.off("TAB001302MenuMove");
	},
    beforeDestroy() {
	...
	methods: {
        ...
        async getSelectOptions(){
			...
			let searchParam = [
				...
				{ menuId:'M0001002', queryId:'selectMaxStockDate', target:stockDate }
            ];
			...
			this.storeValidPassYnList.push({itemValue:"", itemText:"전체"},{itemValue:"Y", itemText:"Y"}, {itemValue:"N", itemText:"N"});            
        ...
        async getDataList(){
			...
			let params = {
				...
				storeValidPassYn: this.searchStoreValidPassYn
			};

			let param = {menuId:'M0001002',queryId:'selectSubMatResinIdStock', queryParams:params, target: this.gridRows};
        ...
        addRow(matInfo){
            ...
            this.matGridDataProvider.addRow({
				...
				unit: matInfo.unit,	
				storeValidDays: matInfo.storeValidDays,
				storeDays: matInfo.storeDays,
        ...
        async processBarcode(){
			...
			if ( this.checkValidBarcodeData() ) {
                ...
                let param = {menuId:'M0001002',queryId:'selectSubMatResinIdStock', queryParams:params, target: matInfo};
        ...
        async excelBtnClick(){
			...
			const fileName = `레진_${yyyymmdd}일_재고현황_${hours}${minutes}${seconds}.xlsx`;			
