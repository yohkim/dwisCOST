...
import gridField from '@web/m0001000/js/TAB001305.js';

const URI_PREFIX = '/api/m0001000/m0001005/TAB001305';
...
export default {
	...
	mounted() {
		...
		this.$eventBus.on("TAB001305MenuMove", this.menuMove);		
	},
	beforeUnmount() {
		this.$eventBus.off("TAB001305MenuMove");
	},
    beforeDestroy() {
	...
	methods: {
        ...
        async getSelectOptions(){
			...
			let searchParam = [
				...
				{ menuId:'M0001005', queryId:'selectMaxStockDate', target:stockDate }
            ];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001005',queryId:'selectSubMatTrayIdStock', queryParams:params, target: this.gridRows};
        ...
        async processBarcode(){
			...
			if ( this.checkValidBarcodeData() ) {
                ...
                let param = {menuId:'M0001005',queryId:'selectSubMatTrayIdStock', queryParams:params, target: matInfo};
        ...
        async excelBtnClick(){
			...
			const fileName = `Tray_${yyyymmdd}일_재고현황_${hours}${minutes}${seconds}.xlsx`;			
