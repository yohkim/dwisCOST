...
import gridField from '@web/m0001000/js/TAB001407.js';

const URI_PREFIX = '/api/m0001000/m0001007/TAB001407';
...
export default {
	...
	mounted() {
		...
		this.matGridView.layoutByColumn("step").spanCallback = this.spanCallbackStep;
	...
	methods: {
        ...
        async getSelectOptions(){
			...
			let searchParam = { menuId:'M0001007', queryId:'selectMaxStockDate', target:stockDate }
		...
		spanCallbackStep(grid, layout, itemIndex) {
			let value = grid.getValue(itemIndex, "step");
			if ( value == "Total") {
				return 2;
			}
			return 1;
		...
		async getDataList(){  
			...
			let param = {menuId:'M0001007',queryId:'selectSubMatFilterMonthReport', queryParams:params, target: this.gridRows};
        ...
        excelBtnClick(){
			...
			const fileName = `필터_${this.searchStockMonth}월_재고현황_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
