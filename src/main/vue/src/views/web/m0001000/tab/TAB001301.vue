    ...
    <div>
        <div class="search_box" :class="{ disabled: isBarcodeMode }">
            <b-row class="search_area">
                ...
                <b-col cols="2">
                    <div class="form-floating">
                        <select class="form-select label-80 " id="floatingSelect" aria-label="Floating label select example" v-model="searchGlassMaterial" >
                            <option value="" >전체</option>
							<option v-for="item in glassMaterialList" :value="item" :key="item">{{item}}</option>
                        </select>
                        <label for="floatingSelect" class="select">원장사양</label>
                    </div>
                </b-col>
                <b-col cols="1">
                    <div class="form-floating">
                        <select class="form-select label-60 " id="floatingSelect" aria-label="Floating label select example" v-model="searchGlassThick" >
                            <option value="" >전체</option>
							<option v-for="item in glassThickList" :value="item" :key="item">{{item}}</option>
                        </select>
                        <label for="floatingSelect" class="select">두께</label>
                    </div>
                </b-col>
                <b-col cols="1">
                    <div class="form-floating">
		...
		<div class="worker_wrap">
            <b-row class="worker_info">	
				<b-col cols="2" class="d-flex ms-3">			
                ...
                <b-col cols="4">
                    <div class="form-floating">
                        <input type="text" ref="glassId" class="form-control label-60" id="floating" placeholder="바코드" autocomplete="off" v-model="glassId" :disabled="isDisabledBarcode" 
...
import gridField from '@web/m0001000/js/TAB001301.js';

const URI_PREFIX = '/api/m0001000/m0001001/TAB001301';
const MAT_CTG_CODE = '10';
...
export default {
	...
	mounted() {
		...
		this.$eventBus.on("TAB001301MenuMove", this.menuMove);
	...
	beforeUnmount() {
		this.$eventBus.off("TAB001301MenuMove");
	},
    beforeDestroy() {
	...
	methods: {
        ...
        menuMove(data) {
            this.$nextTick(()=>{
                ...
                this.searchUseCategory = data.useCategory;
				this.searchGlassMaterial = ( data.glassMaterial ===  "Total" )? "" : data.glassMaterial;
				this.searchGlassThick = ( data.glassThickness ===  "Total" )? "" : data.glassThickness;
        ...
        async getSelectOptions(){
			...
			let params = [ { majCode : "74"}, {matTypeCode : MAT_TYPE_CODE} ];
			let glassMaterial = [];
			let stockDate = [];
			...
			let searchParam = [
				...
				{ menuId:'M0001001', queryId:'selectGlassMaterialList', target:this.glassInfo },
				{ menuId:'M0001001', queryId:'selectMaxStockDate', target:stockDate }
            ];
			...
			this.glassMaterialList = [...new Set(this.glassInfo.map(item=>item.glassMaterial))];
			this.glassThickList = [...new Set(this.glassInfo.map(item=>item.glassThickness))].sort((a,b)=>a-b);
        ...
        async getDataList(){
			...
			let params = {
				...
				glassMaterial : this.searchGlassMaterial,
				glassThick : this.searchGlassThick,
				confirmYn: this.searchConfirmYn,
				diffYn: this.searchDiffYn
			};

			let param = {menuId:'M0001001',queryId:'selectRawMatIdStock', queryParams:params, target: this.gridRows};
		...
		onCellEdited(grid, itemIndex, row, field) {
			...
			//grid.setValue(itemIndex, "confirmYn", "Y");
        ...
        addRow(matInfo){
            ...
            this.matGridDataProvider.addRow({
				...
				glassId: matInfo.glassId,
				useCategory: matInfo.useCategory,		
				useCategoryName: matInfo.useCategoryName,			
				glassMaterial: matInfo.glassMaterial,	
				glassThickness: matInfo.glassThickness,		
				unit: matInfo.unit,	
				prevStockQty: matInfo.prevStockQty,		
				prevRealStockQty: matInfo.prevRealStockQty,			
				inQty: matInfo.inQty,				
				outQty: matInfo.outQty,		
				goodQty: matInfo.goodQty,					
				errorQty: matInfo.errorQty,				
				stockQty: matInfo.stockQty,					
				realStockQty: matInfo.realStockQty,				
				diffQty: matInfo.diffQty,					
				confirmYn: "Y", //matInfo.confirmYn,			
            ...
            //this.matGridView.setCurrent({ itemIndex: itemIndex, fieldName: 'realStockQty'  });
			//this.matGridView.showEditor();

         	this.glassId = '';
            this.$refs.glassId.focus();
        },  
        ...
        async processBarcode(){
			...
			if ( this.checkValidBarcodeData() ) {
                ...
                let param = {menuId:'M0001001',queryId:'selectRawMatIdStock', queryParams:params, target: matInfo};
        ...
        async excelBtnClick(){
			...
			const fileName = `원자재_${yyyymmdd}일_재고현황_${hours}${minutes}${seconds}.xlsx`;			
