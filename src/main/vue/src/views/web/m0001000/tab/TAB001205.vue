        ...
        <div class="search_box" :class="{ disabled: isBarcodeMode }">
            <b-row class="search_area">
                ...
                <b-col cols="1">
                    <div class="form-floating">
                        ...
                        <label for="floatingSelect" class="select">작업조</label>
                    </div>
                </b-col>
                <b-col cols="2">
                    <div class="form-floating">
         ...
         <div class="worker_wrap">
            <b-row class="worker_info">
                ...
                <b-col cols="2">
                    <div style="width:400px"><span style="font-size: 11px; color: blue;">* 공정투입 작업전 작업조/투입설비 선택, 작업자를 입력하십시요. </span></div>
                </b-col>
            </b-row>
        </div>
...
import gridField from '@web/m0001000/js/TAB001205.js';

const URI_PREFIX = '/api/m0001000/m0001005/TAB001205';
...
export default {
	...
	computed: {
        ...
        isDisabledBarcode(){
            return _.isEmpty(this.searchShift) ||  _.isEmpty(this.searchEquip) || _.isEmpty(this.workerName);
        },
        isBarcodeMode(){
	...
	mounted() {
        ...
        this.$eventBus.on("TAB001205MenuMove", this.menuMove);            
	},

	beforeUnmount() {
        this.$eventBus.off("TAB001205MenuMove");
    },
    beforeDestroy() {
    ...
    methods: {
        ...
        async getSelectOptions(){
			...
			let params = [ { majCode : "06"}, { line : 'DFB1', stepCode : ['061','073']}, {matCategoryCode : MAT_CTG_CODE, matTypeCode : MAT_TYPE_CODE} ];
            let matInfo = [];
			let equipList = [];
			...
			let searchParam = [
				...
				{ menuId:'M0001005', queryId:'selectUseEquipList', queryParams:params[1], target:equipList },
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001005',queryId:'selectMatTrayOut', queryParams:params, target: this.gridRows};
        ...
        setCellStyleCallbackMatGrid(grid, cell){
            ...
            if ( fieldName === "workerName" ||  fieldName === "equipNm" ||  fieldName === "useQty" || fieldName === "errorQty" || fieldName === "comments" ) {
        ...
        onCellClicked( grid, clickData ){
			...
			let searchParams = { line : 'DFB1', stepCode : ['061','073']};
            ...
            if ( clickData.editable ) { 
                ...
                if (clickData.fieldName === "equipNm" || clickData.fieldName === "stepNm"){
                    let params = {
                        ...
                        search: { menuId: "M0001005", queryId: "selectUseEquipList", queryParams: searchParams }
                    };	
        ...
        onValidateColumn(grid, column, inserting, value, itemIndex, dataRow) {
            ...
            if (column.fieldName === "useQty" || column.fieldName === "errorQty"   ) {
                const useQty = grid.getValue(itemIndex, "useQty") ;
				const errorQty = grid.getValue(itemIndex, "errorQty") ;
                if ( ( useQty + errorQty ) > stockQty ){
					error.level = "warning";
        ...
        async processBarcode(){
			...
			if ( this.checkValidBarcodeData() ) {
                ...
                let param = {menuId:'M0001005',queryId:'selectBarcodeInfo', queryParams:params, target: matInfo};
        ...
        async excelBtnClick(){
			...
			const fileName = `Tray_공정투입_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
