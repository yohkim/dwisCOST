    ...
    <div>
        ...
        <div class="worker_wrap">
            <b-row class="worker_info">
                ...
                <b-col cols="3">
                    <div class="form-floating">
                        <input type="text" ref="materialId" class="form-control label-60" id="floating" placeholder="바코드" autocomplete="off"
                         v-model="materialId" 
                         :disabled="this.searchSubMatType==='' || this.workerName==='' " 
                ...
                <b-col cols="2">
                    <div style="width:300px"><span style="font-size: 11px; color: blue;">* 입고작업전 Size 선택, 담당자를 입력하십시요. </span></div>
                </b-col>                                   
            </b-row>
        </div>
...
import gridField from '@web/m0001000/js/TAB001108.js';

const URI_PREFIX = '/api/m0001000/m0001008/TAB001108';
...
export default {
	...
	computed: {
        ...
        selectedSizeText() {
            let selected = this.subMatTypeList.find(item => item.itemValue === this.searchSubMatType);
            return selected ? selected.itemText : "";
	...
	mounted() {   
        ...
        this.getDataList();	 
         ...
         this.$eventBus.on("TAB001108MenuMove", this.menuMove);   
	},

	beforeUnmount() {
        this.$eventBus.off("TAB001108MenuMove");
    },
    beforeDestroy() {
    ...
    methods: {
        ...
        async getSelectOptions(){
			...
			let params = [ { majCode : "92"}, {matCategoryCode : MAT_CTG_CODE, matTypeCode : MAT_TYPE_CODE} ];
            let matInfo = [];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001008',queryId:'selectMatDummyIn', queryParams:params, target: this.gridRows};
		...
		onCellEdited(grid, itemIndex, row, field) {
            ...
            if ( fieldName === "SUBMATTYPECODE" ) {
                ...
                grid.setValue(itemIndex, "size", displayValues.subMatTypeCode);
            }
		},        
        ...
        async processBarcode(){
            ...
            if(_.isEmpty(this.searchSubMatType)){
                this.$toast('info', "Size를 선택하십시요!");
                return false;
            }
			...
			let param = {menuId:'M0001008',queryId:'selectBarcodeDupYn', queryParams:params, target: dupYn};
        ...
        async excelBtnClick(){
			...
			const fileName = `더미글라스_공정전입고_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
