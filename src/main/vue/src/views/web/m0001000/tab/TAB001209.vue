        ...
        <div class="search_box" :class="{ disabled: isBarcodeMode }">
         ...
         <div class="worker_wrap">
            <b-row class="worker_info">
                ...
                <b-col cols="2">
                    <div style="width:400px"><span style="font-size: 11px; color: blue;">* 공정투입 작업전 자재분류/품명 선택, 작업자를 입력하십시요. </span></div>
                </b-col>
            </b-row>
        </div>
...
import gridField from '@web/m0001000/js/TAB001209.js';

const URI_PREFIX = '/api/m0001000/m0001009/TAB001209';
const P_STEP = ['074', '075', '077', '080', '081'];
...
export default {
	...
	mounted() {
        ...
        this.$eventBus.on("TAB001209MenuMove", this.menuMove);            
	},
	beforeUnmount() {
        this.$eventBus.off("TAB001209MenuMove");
    },
    beforeDestroy() {
    ...
    methods: {
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001009',queryId:'selectMatEtcOut', queryParams:params, target: this.gridRows};
        ...
        onCellClicked( grid, clickData ){
            ...
            if ( clickData.editable ) { 				
                ...
                if (clickData.matCategoryCode === "60" ) {
                    step = [...P_STEP];    
                }				
		...
		async addBtnClick() {
			...
			if ( this.checkValidAddData() ) {			
                ...
                let param = {menuId:'M0001009',queryId:'selectEtcInfo', queryParams:params, target: matInfo};
        ...
        cancelBtnClick() {
                ...
                this.$confirm("확인", "자재사용을 취소하시겠습니까?", (confirm)=>{
                        if(confirm){                            
                            this.getDataList();
                        }
                    });  
        ...
        async excelBtnClick(){
			...
			const fileName = `${this.selectedMatCategoryName}_${this.selectedMatTypeName}_공정사용_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
