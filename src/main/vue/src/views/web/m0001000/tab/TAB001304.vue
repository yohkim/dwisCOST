...
import gridField from '@web/m0001000/js/TAB001304.js';

const URI_PREFIX = '/api/m0001000/m0001004/TAB001304';
...
export default {
	...
	mounted() {
		...
		this.$eventBus.on("TAB001304MenuMove", this.menuMove);		
	},
	beforeUnmount() {
		this.$eventBus.off("TAB001304MenuMove");
	},
    beforeDestroy() {
	...
	methods: {
        ...
        async getSelectOptions(){
			...
			let searchParam = [
				...
				{ menuId:'M0001004', queryId:'selectMaxStockDate', target:stockDate }
            ];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001004',queryId:'selectSubMatFilmIdStock', queryParams:params, target: this.gridRows};
        ...
        async processBarcode(){
			...
			if ( this.checkValidBarcodeData() ) {
                ...
                let param = {menuId:'M0001004',queryId:'selectSubMatFilmIdStock', queryParams:params, target: matInfo};
        ...
        async excelBtnClick(){
			...
			const fileName = `필름_${yyyymmdd}일_재고현황_${hours}${minutes}${seconds}.xlsx`;			
