...
import gridField from '@web/m0001000/js/TAB001308.js';

const URI_PREFIX = '/api/m0001000/m0001008/TAB001308';
...
export default {
	...
	mounted() {
		...
		this.$eventBus.on("TAB001308MenuMove", this.menuMove);		
	},
	beforeUnmount() {
		this.$eventBus.off("TAB001308MenuMove");
	},
    beforeDestroy() {
	...
	methods: {
        ...
        async getSelectOptions(){
			...
			let searchParam = [
				...
				{ menuId:'M0001008', queryId:'selectMaxStockDate', target:stockDate }
			];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001008',queryId:'selectSubMatDummyIdStock', queryParams:params, target: this.gridRows};
        ...
        addRow(matInfo){
            ...
            this.matGridDataProvider.addRow({
				...
				errorWorkQty: matInfo.errorWorkQty,
				errorMatQty: matInfo.errorMatQty,					
        ...
        async processBarcode(){
			...
			if ( this.checkValidBarcodeData() ) {
                ...
                let param = {menuId:'M0001008',queryId:'selectSubMatDummyIdStock', queryParams:params, target: matInfo};
        ...
        async excelBtnClick(){
			...
			const fileName = `더미슬라스_${yyyymmdd}일_재고현황_${hours}${minutes}${seconds}.xlsx`;			
