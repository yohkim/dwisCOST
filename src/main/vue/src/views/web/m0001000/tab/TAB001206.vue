...
import gridField from '@web/m0001000/js/TAB001206.js';

const URI_PREFIX = '/api/m0001000/m0001006/TAB001206';
...
export default {
	...
	mounted() {
        ...
        this.$eventBus.on("TAB001206MenuMove", this.menuMove);
	},

	beforeUnmount() {
        this.$eventBus.off("TAB001206MenuMove");
    },
    beforeDestroy() {
    ...
    methods: {
        ...
        async getSelectOptions(){
			...
			let params = [ { majCode : "80"}, { majCode : "75"}, { line : 'DFB1', stepCode : step}, {matCategoryCode : MAT_CTG_CODE, matTypeCode : MAT_TYPE_CODE} ];
            let matInfo = [];
			let equipList = [];
            let shiftInfo = [];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001006',queryId:'selectMatChemicalOut', queryParams:params, target: this.gridRows};
        ...
        setCellStyleCallbackMatGrid(grid, cell){
            ...
            if ( fieldName === "workerName" ||  fieldName === "equipNm" || fieldName === "comments") {
        ...
        onCellClicked( grid, clickData ){
            ...
            if ( clickData.editable ) { 				
				...
				let step = [];
				let subMatTyepCode = grid.getValue(clickData.itemIndex,"subMatTypeCode");
				if ( subMatTyepCode === 'E') {
					step = [...E_STEP];
				}
				else if ( subMatTyepCode === 'D' ) {
					step = [...D_STEP];
				}
				else if ( subMatTyepCode === 'W'  ) {
					step = [...W_STEP];
				}
				else {
				...
				let searchParams = { line : 'DFB1', stepCode : step, subMatTyepCode : subMatTyepCode };
        ...
        async processBarcode(){
			...
			if ( this.checkValidBarcodeData() ) {
                ...
                let param = {menuId:'M0001006',queryId:'selectBarcodeInfo', queryParams:params, target: matInfo};
                ...
                if ( _.isEmpty(resp) ) {
					...
					else if ( !_.isEmpty(resp[0].returnNo)) {
						this.$toast("info", "반납확인이 안된 내역이 있습니다.\n반납확인 후 공정투입 가능합니다!");
						this.materialId = "";
                        return false;            
					}
                    else {
		...
		async saveData() {
			...
			if ( this.checkValidSaveData(saveData) ) {
				...
				await this.$axios.post(URI_PREFIX + "/saveOutData", param).then((response) => {
					...
					else if ( response.data.resultCode === "RETURN_CONFIRM") {

                        const msg = `[바코드(자재ID) 확인]\n[${response.data.resultData}] \n반납확인이 되지 않았습니다. 반납 확인 처리후 진행하십시요!` ;
						this.$toast("info", msg);
					}												
				}).catch((error) => {
        ...
        async excelBtnClick(){
			...
			const fileName = `약액_공정투입_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
