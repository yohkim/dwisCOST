...
import gridField from '@web/m0001000/js/TAB001103.js';

const URI_PREFIX = '/api/m0001000/m0001003/TAB001103';
...
export default {
	...
	data () {
		return {
            ...
            subMatTypeList: [],
            curMode: 'R',			
            searchFromDate: this.$utils.getWorkDate("-"), //this.$utils.getSomeMonthDate(-1),
	...
	mounted() {
        ...
        this.$eventBus.on("TAB001103MenuMove", this.menuMove);
	},

	beforeUnmount() {
        this.$eventBus.off("TAB001103MenuMove");
    },
    beforeDestroy() {
    ...
    methods: {
        ...
        async getSelectOptions(){
			...
			let params = [ { majCode : "79"}, {matCategoryCode : MAT_CTG_CODE, matTypeCode : MAT_TYPE_CODE} ];
            let matInfo = [];
        ...
        async getDataList(){
			...
			let param = {menuId:'M0001003',queryId:'selectMatToolIn', queryParams:params, target: this.gridRows};
        ...
        setCellStyleCallbackMatGrid(grid, cell){
            ...
            if ( fieldName === "comments" || fieldName === "subMatTypeCode" || fieldName === "workerName") {
        ...
        async processBarcode(){
			...
			let param = {menuId:'M0001003',queryId:'selectBarcodeDupYn', queryParams:params, target: dupYn};
        ...
        async excelBtnClick(){
			...
			const fileName = `Tool_공정전입고_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
