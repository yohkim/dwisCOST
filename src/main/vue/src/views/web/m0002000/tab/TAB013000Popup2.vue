...
export default {
  ...
  created() {
    const gridModule = require(`../js/TAB013000Popup2.js`);
  ...
  methods: {
      ...
      this.$nextTick(()=>{
        ...
        this.$axios.api.search(Object.assign(this.params.search, {target: this.rows})).then(()=>{
          let nextStep= this.params.clickRow['다음공정코드'];
          if ( !_.isEmpty(nextStep) ) { 
              let steps = this.params.clickRow['다음공정코드'].split(',');
              steps.forEach(element => {
                let index = this.rows.findIndex(rowElem=>{return rowElem['공정코드']==element});
                this.cmDialog1GridGV.checkRow(index);
              });
          }
          this.isOpen = true;
        });
