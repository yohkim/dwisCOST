    ...
    <div class="grid_box search_onerow">
      <div class="left_box">
        <div class="btn_wrap ms-auto">
          <b-button @click="addRow" class="sub"><span></span>행추가</b-button>
          <b-button @click="saveRows" class="sub"><span></span>HOLD</b-button>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      holdGrid: null,
      rows: [],
      editableColumns: ["runNo", "hold사유", "비고",'hold명'],
      runNo:''
    };
  },
  computed: {
  ...
  methods: {
    ...
    menuMove(data) {
      this.$nextTick(() => {
        ...
        if (data != null && !_.isEmpty(data['수주일자'])) {
          let date = this.$utils.rgDisplayDate(null, null, data['수주일자']);
          this.startDate = date;
          this.endDate = date;
        } else if (data != null && !_.isEmpty(data['startDate']) && !_.isEmpty(data['endDate'])) {
    ...
    addRow(){
      this.GV.commit();
			this.DP.addRow({"담당자":this.userAuthInfo.userInfo.userName});
