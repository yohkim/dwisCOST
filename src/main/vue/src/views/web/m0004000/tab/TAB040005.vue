  ...
  <div>
    ...
    <div class="grid_box search_onerow">
      ...
      <div class="grid-border-none">
        <RealGrid ref="lotDetailChangeGrid" :uid="'lotDetailChangeGrid'" :rows="rows" style="height: 100%" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    GV() {
      return this.$refs.lotDetailChangeGrid.getGridView();
    },
    DP() {
      return this.$refs.lotDetailChangeGrid.getGridDataProvider();
    },
  },
  created() {
    this.initializeGrid();
  ...
  methods: {
    ...
    async initializeGrid() {
      this.lotDetailChangeGrid = _.cloneDeep(require(`@web/m0004000/js/TAB040005.js`));
    },
    ...
    async excelClick() {
      ...
      const fileName = `LotNo변경관리상세_${yyyymmdd}_${hours}${minutes}${seconds}.xlsx`;
      let that = this;
      const options = {
        type: 'excel',
        target: 'local',
    ...
    onDataLoadComplatedLotDetailChangeGrid(grid) {
      grid.fitLayoutWidth(null);
      grid.fitLayoutWidth('구lotNo', null, 150, true);
      grid.fitLayoutWidth('신lotNo', null, 150, true);
      grid.fitLayoutWidth('구lotpono', null, 150, true);
      grid.fitLayoutWidth('신lotpono', null, 150, true);
    },
  },
};
</script>
