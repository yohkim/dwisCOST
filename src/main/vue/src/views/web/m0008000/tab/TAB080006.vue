...
export default {
  ...
  data() {
    return {
      ...
      menuId: 'M0008002',
      inspectionQueryMap: {
        retry: 'M0008002_U_Inspection2',
        confirm: 'M0008002_U_Inspection3',
      },
      params: {
        user: null,
        cellId: null,
  ...
  methods: {
    ...
    initializeGrid() {
      this.grid = _.cloneDeep(require(`@web/m0008000/js/TAB080006.js`));
    },
    ...
    async saveData() {
      ...
      saveData.update.forEach((item) => {
        if (this.oneJudge.includes(this.srchInfo.menu2Item.value) && !this.$utils.isRequiredFieldsFilled(item, this.oneCheckList)) {
          isEmpty = true;
        } else if (!this.oneJudge.includes(this.srchInfo.menu2Item.value) && !this.$utils.isRequiredFieldsFilled(item, this.twoCheckList)) {
          isEmpty = true;
        }
      });
      if (isEmpty) {
