...
export default {
  ...
  methods: {
    ...
    initializeGrid() {
      this.grid = _.cloneDeep(require(`@web/m0008000/js/TAB080005.js`));
    },
    ...
    resetGrid() {
      ...
      } else if (this.twoJudge.includes(this.srchInfo.menu2Item.value)) {
        if (this.gridView.columnByName('판정2') != null) {
          this.gridView.columnByName('판정2').editable = true;
        }
        if (this.gridView.columnByName('판정1') != null && this.gridView.columnByName('판정1').header != null) {
    ...
    async actionClick(action) {
      if (action == 'start' && !this.isStartProcessing) {
        ...
        let param = {
          menuId: this.menuId,
          update: [{ queryId: 'M0008002_Update2', data: saveData.update }],
        };
        ...
        let isEmpty = false;
        rows.forEach((dataRow) => {
          if (this.oneJudge.includes(this.srchInfo.menu2Item.value) && !this.$utils.isRequiredFieldsFilled(jsonRows[dataRow], this.oneCheckList)) {
            isEmpty = true;
          } else if (!this.oneJudge.includes(this.srchInfo.menu2Item.value) && !this.$utils.isRequiredFieldsFilled(jsonRows[dataRow], this.twoCheckList)) {
            isEmpty = true;
          }
        });
        ...
        let param = {
          menuId: this.menuId,
          update: [{ queryId: 'M0008002_Update3', data: saveData.update }],
        };
        await this.$axios.api.saveData(param);
    ...
    updateResultValue() {
      ...
      let params = {
        ...
        tabId: this.tabId,
        gridJs: this.twoJudgeDiffHeader.includes(this.srchInfo.menu2Item.value) ? 'TAB080002DiffHeaderPopup' : 'TAB080002Popup',
      };
    ...
    handleUpdateValueConfirm(params) {
      ...
      rows.forEach((dataRow) => {
        if (this.dataProvider.getValue(dataRow, '진행상태') != '검사완료') {
          if (this.twoJudge.includes(this.srchInfo.menu2Item.value) || this.twoJudgeDiffHeader.includes(this.srchInfo.menu2Item.value)) {
    ...
    fileAttach() {
      ...
      let curGroupId = 'M0008002' + '_' + this.srchInfo.menu2Item.value + '_' + inspectionUser;
