    ...
    <div class="grid_box material_box">
      <div class="left_box">
        <div class="btn_wrap ms-auto">
          <b-button class="second" @click="openHoldClick">HOLD</b-button>
...
export default {
  ...
  methods: {
    ...
    initializeGrid() {
      this.grid = _.cloneDeep(require(`@web/m0008000/js/TAB080002.js`));
    },
    ...
    async actionClick(action) {
      if (action == 'start' && !this.isStartProcessing) {
        ...
        let param = {
          menuId: this.menuId,
          update: [{ queryId: 'M0008001_Update2', data: saveData.update }],
        };
        ...
        let isEmpty = false;
        rows.forEach((dataRow) => {
          if (this.oneJudge.includes(this.srchInfo.menu1Item.value) && !this.$utils.isRequiredFieldsFilled(jsonRows[dataRow], this.oneCheckList)) {
            isEmpty = true;
          } else if (!this.oneJudge.includes(this.srchInfo.menu1Item.value) && !this.$utils.isRequiredFieldsFilled(jsonRows[dataRow], this.twoCheckList)) {
            isEmpty = true;
          }
        });
        ...
        let param = {
          menuId: this.menuId,
          update: [{ queryId: 'M0008001_Update3', data: saveData.update }],
        };
        await this.$axios.api.saveData(param);
    ...
    updateResultValue() {
      ...
      let params = {
        ...
        tabId: this.tabId,
        gridJs: this.twoJudgeDiffHeader.includes(this.srchInfo.menu1Item.value) ? 'TAB080002DiffHeaderPopup' : 'TAB080002Popup',
      };
    ...
    handleUpdateValueConfirm(params) {
      ...
      rows.forEach((dataRow) => {
        if (this.dataProvider.getValue(dataRow, '진행상태') != '검사완료') {
          if (this.twoJudge.includes(this.srchInfo.menu1Item.value) || this.twoJudgeDiffHeader.includes(this.srchInfo.menu1Item.value)) {
