    ...
    <div class="grid_box material_box">
      <div class="left_box">
        <div class="btn_wrap ms-auto">
          <b-button class="main" @click="inspectionClick">회수처리</b-button>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      menuId: 'M0007004',
      searchQueryId: 'M0007004_Sch2',
      inspectionQueryId: 'M0007004_U_Inspection2',
      params: {
        step: null,
        type: null,
        model: null,
  ...
  mounted() {
    this.cstGridView.setCheckableExpression("values['카세트상태'] != '반출' ", true);
  },
  beforeUnmount() {},
  methods: {
    initialize() {
    ...
    initializeGrid() {
      this.cstGrid = _.cloneDeep(require(`@web/m0007000/js/TAB074002.js`));
    },
    ...
    async inspectionData() {
      ...
      if (_.isEmpty(this.dept) || _.isEmpty(this.inspectionCharger)) {
        this.$toast('info', '부서와 담당자를 입력 후 다시 회수처리를 클릭하세요.');
        return;
      }
      ...
      rows.forEach((dataRow) => {
        this.cstGridDataProvider.setValue(dataRow, '회수일', cDate);
        this.cstGridDataProvider.setValue(dataRow, '회수시간', cTime);
        this.cstGridDataProvider.setValue(dataRow, '회수자부서', this.dept);
        this.cstGridDataProvider.setValue(dataRow, '회수자', this.inspectionCharger);
      });
      this.cstGridView.commit();
      let jsonRows = this.cstGridDataProvider.getJsonRows(0, -1);
      const updated = [];
      rows.forEach((dataRow) => {
        if ('반출대기' == this.cstGridDataProvider.getValue(dataRow, '카세트상태')) {
          updated.push(jsonRows[dataRow]);
        }
      });

      if (updated.length == 0) {
        this.$toast('info', '모두 반출대기 상태가 아닙니다.');
        return;
      }
      ...
      if (resp.status === 'success') {
        this.$toast('success', `반출 상태로 ${updated.length}건 변경되었습니다.`);
        result = true;
      } else {
