...
import com.dowinsys.mes.web.m0001000.mapper.M0001005Mapper;
import com.dowinsys.mes.web.m0001000.service.M0001005Service;

@Service("com.dowinsys.mes.web.m0001000.service.M0001005")
public class M0001005ServiceImpl implements M0001005Service {

	@Autowired
	M0001005Mapper mapper;
...
/*						checkData = mapper.checkDeleteInData(deleteData.getData());
	...
	public Map<String, Object> saveSummaryData(Map<String, Object> params) {
		...
		try 
		{
			...
			if ( preStockStatus == null || preStockStatus.isEmpty() ) {
				...
				String tempTableName = "temp_sub_mat_tray_id_stock_" + params.get("stockDate") + "_" + UUID.randomUUID().toString().replace("-", "").substring(0, 8);
				params.put("tempTableName", tempTableName);
				
				mapper.createTempMatStockTable(params);				
			
				mapper.deleteSubMatTrayIdStock(params);
				mapper.insertSubMatTrayIdStock(params);
				mapper.deleteSubMatTrayStock(params);
				mapper.insertSubMatTrayStock(params);
	...
	public Map<String, Object> saveStockIdData(GenericResourceVo grv) {
		...
		try {
				...
				for (QueryData insertData : grv.getInsert()) {
					...
					if (insertData.getData() != null && !insertData.getData().isEmpty()) {
						...
						for (Map<String, Object> data : insertData.getData()) {
							...
							mapper.updateSubMatTrayIdStock(data); 				
						}
					}
				}
			}			
			...
			if ( reSummary ) {			
				mapper.deleteSubMatTrayStock(grv.getQueryParams());
				mapper.insertSubMatTrayStock(grv.getQueryParams());
			}
	...
	@Transactional
	public Map<String, Object> saveTrayModelData(GenericResourceVo grv) {
		...
		try {
			...
			if (grv.getInsert() != null) {
				for (QueryData insertData : grv.getInsert()) {
					...
					if (insertData.getData() != null && !insertData.getData().isEmpty()) {
						checkData = mapper.checkTrayCodeData(insertData.getData());

						if (checkData != null && !checkData.isEmpty()
								&& !"NoData".equals(checkData.get(0).get("trayCode"))) {

							resultData.put("resultCode", "DUP_CODE");
							resultData.put("resultData", checkData.get(0).get("trayCode"));
